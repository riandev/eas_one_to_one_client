...
const BrReport = () => {
  ...
  const handleBrReport = () => {
    fetch(`http://localhost:5010/baReport/${brDate}`)
      .then((res) => res.json())
      .then((data) => setStatus(data));
  };
  const handleTerritoryReport = () => {
    fetch(`http://localhost:5010/territoryReport/${territoryDate}`)
      .then((res) => res.json())
      .then((data) => setTerritoryStatus(data));
  };
  const handleAreaReport = () => {
    fetch(`http://localhost:5010/areaReport/${areaDate}`)
      .then((res) => res.json())
      .then((data) => setAreaStatus(data));
  };
  const handleRegionReport = () => {
    fetch(`http://localhost:5010/regionReport/${regionDate}`)
      .then((res) => res.json())
