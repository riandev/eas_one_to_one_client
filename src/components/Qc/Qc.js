...
const Qc = () => {
  ...
  const handleSearch = () => {
    fetch(`http://192.168.10.11:5010/dMatched/${qcNumber}`)
      .then((res) => res.json())
      .then((data) => setMatchedQC(data));
  };
  const handleUpdate = (id) => {
    console.log(id);
    fetch(`http://192.168.10.11:5010/update/${id}`)
      .then((res) => res.json())
        ...
        <table className="table table-borderless table-hover">
          ...
          <tbody>
            <tr>
              ...
              <td>{matchedQC?.answer7dot1}</td>
              <td>{matchedQC?.answer7dot1dot1}</td>
              <td>{matchedQC?.answer8dot1}</td>
              <td>{matchedQC?.answer8dot2}</td>
